...
export class CreateOccurrenceDTO {
  ...
  constructor(data: any) {
    ...
    this.occurrenceDate = new Date(data.occurrenceDate);
    this.activationDate = new Date(data.activationDate);
  ...
  validate(): string[] {
    ...
    // Tipo
    if (!this.type) {
      errors.push('Tipo da ocorrência é obrigatório');
    } else if (!OCCURRENCE_TYPES.includes(this.type as any)) {
    ...
    } else if (this.municipality.length < 2) {
